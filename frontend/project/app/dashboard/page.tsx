...
import {
  ...
  ResponsiveContainer,
  CartesianGrid,
  Rectangle, // Import Rectangle for the custom bar
} from "recharts";
...
const LeafIcon = () => (
  <svg
    ...
    className="inline-block h-8 w-8 text-green-400"
    viewBox="0 0 20 20"
    fill="currentColor"
  >
    <path
      fillRule="evenodd"
      d="M10 2a8 8 0 100 16 8 8 0 000-16zM5.586 8.586a2 2 0 112.828 2.828L10 10l-2.828-2.828-1.586 1.586zM10 10l1.414 1.414a2 2 0 11-2.828-2.828L10 10z"
      clipRule="evenodd"
    />
  </svg>
);
...
// A custom component for the bar that's being hovered over
const CustomActiveBar = (props: any) => {
  ...
  return (
    <Rectangle
      {...props}
      x={x}
      y={y - 2} // Slightly raise the bar
      width={width}
      height={height + 4} // Slightly increase height
      fill="#86efac" // A brighter green
      stroke="#bef264"
      strokeWidth={2}
      radius={[8, 8, 0, 0]}
    />
...
// A custom component for the tooltip content
    ...
    return (
      <div className="p-4 bg-gray-900/90 flex flex-col gap-2 rounded-xl border border-green-500 shadow-lg">
        ...
        <p className="text-sm text-white">
          Projects:
          <span className="ml-2 font-bold text-lg">{payload[0].value}</span>
        </p>
      </div>
    );
  }
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    if (!role) {
      setContent(
        <p className="text-red-500 font-semibold animate-pulse">
          No role selected! Please log in.
        </p>
      );
      return;
    }
    ...
    switch (role) {
      case "government":
        setContent(<GovernmentDashboard />);
        break;
      case "startup":
        setContent(<StartupDashboard />);
        break;
      case "bank":
        setContent(<BankDashboard />);
        break;
      case "auditor":
        setContent(<AuditorDashboard />);
        break;
      default:
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-blue-950 to-green-950 text-white p-4 md:p-8 font-sans">
      <header className="flex flex-col md:flex-row justify-between items-center mb-8 bg-black/30 backdrop-blur-sm p-4 rounded-xl shadow-lg border border-green-700/50">
        ...
        <a
          href="/login"
          className="px-6 py-2 rounded-full bg-red-600 hover:bg-red-700 transition-all duration-300 ease-in-out shadow-md hover:shadow-lg transform hover:-translate-y-0.5 text-lg font-medium"
        >
          Logout
        </a>
      </header>
...
function ActivityTable() {
  const rows = [
    { id: 1, name: "Hydrogen Plant A", status: "Approved" },
    { id: 2, name: "Fuel Cell R&D", status: "Pending" },
    { id: 3, name: "Electrolyzer Project", status: "Rejected" },
    { id: 4, name: "Green Ammonia Synthesis", status: "Approved" },
    { id: 5, name: "Hydrogen Storage Facility", status: "Pending" },
  ];
  return (
        ...
        <table className="min-w-full text-left border-collapse">
          ...
          <tbody>
            {rows.map((r, index) => (
              <tr
                key={r.id}
                className={`border-b border-gray-700/50 hover:bg-white/5 transition-colors duration-200 ${
